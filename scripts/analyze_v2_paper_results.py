...
Reads outputs/v2/paper_results/all_episodes.csv.
Generates:
  - LaTeX tables in outputs/v2/paper_tables/
  - Figures (PNG 300dpi + PDF) in outputs/v2/paper_figures/
...
INPUT_CSV = "outputs/v2/paper_results/all_episodes.csv"
TABLE_DIR = "outputs/v2/paper_tables"
...
def fig_boxplot_path_length(df: pd.DataFrame) -> None:
    ...
    for ax, diff in zip(axes, DIFFICULTY_ORDER):
        ...
        bp = ax.boxplot(
            data,
            labels=[_label(p) for p in PLANNER_ORDER],
            patch_artist=True,
            widths=0.6,

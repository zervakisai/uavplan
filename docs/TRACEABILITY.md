...
| FC-1 | Corridor interdictions (fire closures / vehicle roadblocks) on A* corridor, not planner path | `dynamics/fire_ca.py`, `dynamics/traffic.py` | `contract_test_fairness.py` | `outputs/fairness_audit.json` | 5 |
...
| VC-3 | (Removed — interdictions are physical fire/traffic events, no separate lifecycle rendering) | — | — | — | — |
...
| PL-3 | 5 planners in PLANNERS registry | `planners/__init__.py` | `unit_test_planner_registry.py` | — |
...
| `fairness_audit.json` | A* corridor cells vs planner paths; interdiction placement proof | `contract_test_fairness.py` |

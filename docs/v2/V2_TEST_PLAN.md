...
| `test_no_independent_rng_constructors` | DC-1 | No `np.random.RandomState`, `random.Random`, or bare `np.random.seed` in `src/uavbench2/` |
| `test_identical_seed_identical_output` | DC-2 | Two runs with `(gov_civil_protection_easy, astar, seed=42)` produce SHA-256 identical event logs, trajectories, and metrics dicts |
...
| Test Name | Contract | Acceptance Criterion |
|-----------|----------|---------------------|
| `test_interdiction_on_bfs_corridor` | FC-1 | All interdiction cells are on the BFS shortest path computed on the static grid before any planner runs |
...
| `test_all_reject_reasons_exercised` | EC-1 | Over a hard scenario, at least `BUILDING`, `FIRE`, `FORCED_BLOCK` are observed |
...
| `test_depth1_clears_forced_blocks` | GC-1 | After injecting a forced block on the path, guardrail at D1 clears it; `relaxations[0]` documents cells freed |
...
| Test Name | Contract | Acceptance Criterion |
|-----------|----------|---------------------|
| `test_single_definition` | MP-1 | `grep -r "def compute_blocking_mask" src/uavbench2/` returns exactly 1 match |
...
### 2.8 `contract_test_visual_truth.py` — VC-1, VC-2, VC-3

**Contracts tested**: VC-1 (path visible), VC-2 (NO_PLAN/STALE badge), VC-3 (block lifecycle)
...
| `test_forced_block_triggered` | VC-3 | At event_t1, HUD shows forced block TRIGGERED state |
...
| `unit_test_scenario_loader.py` | `scenarios/loader.py` | Load all 9 YAMLs, osm+synthetic map sources, invalid YAML rejection (SC-1, SC-3) |
...
| `unit_test_planner_registry.py` | `planners/__init__.py` | Exactly 6 keys in PLANNERS (PL-3) |
...
| `unit_test_theta_star.py` | `planners/theta_star.py` | Any-angle path, Bresenham LOS, grid expansion |
| `unit_test_dstar_lite_api.py` | `planners/dstar_lite.py` | Incremental update correctness, obstacle change handling (PL-5) |
| `unit_test_mppi_grid.py` | `planners/mppi_grid.py` | Sampling with env-seeded RNG, horizon, sample count |
...
|-----------|-------|---------------------|
| `integration_test_runner_e2e.py` | Full episode: scenario → env → planner → metrics | Episode completes for `(gov_civil_protection_easy, astar, seed=42)` with success; metrics contain all required fields; motion is legal (PC-1); planned vs executed counts differ (PC-2) |
| `integration_test_cli.py` | CLI entry point | `python -m uavbench2 run --scenarios gov_civil_protection_easy --planners astar --trials 1 --seed-base 42 --output-dir /tmp/v2_test` exits 0 and produces JSON output |
| `integration_test_dynamic_episode.py` | Full dynamic episode | `(gov_civil_protection_hard, periodic_replan, seed=42)` completes; dynamics active; guardrail invoked at least once; events contain step_idx |
...
| 7 | `unit_test_theta_star.py` | Unit | — |
| 7 | `unit_test_dstar_lite_api.py` | Unit | PL-5 |
| 7 | `unit_test_mppi_grid.py` | Unit | — |
...
| VC-3 | `contract_test_visual_truth.py` | 3 | 8 |

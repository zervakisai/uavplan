...
**Write**: `docs/v2/V2_DECISIONS.md`, `outputs/v2/rebuild_audit.json`
...
**Write**:
- `docs/v2/V2_REQUIREMENTS.md` — SHALL requirements + acceptance criteria
- `docs/v2/V2_ARCHITECTURE.md` — module map, single pipeline, data contracts
- `docs/v2/V2_TRACEABILITY.md` — Req → Code → Tests → Evidence
- `docs/v2/V2_TEST_PLAN.md` — unit/contract/integration + CI split
- `docs/v2/MISSION_CARDS.md` — mission story cards per scenario family
- `docs/v2/VISUAL_TRUTH_SPEC.md` — HUD tokens, planned path rules, event semantics
...
**Gate 2**: `pytest tests/v2/ -q` passes. Two identical seed runs produce identical hashes.
...
**Gate 8**: `contract_test_visual_truth.py` passes. `outputs/v2/viz_manifest.csv` + `viz_frame_checks.json` written.
...
- `outputs/v2/repro_manifest.json` — one command regenerates everything
**Gate 9**: Single command produces deterministic artifacts. `outputs/v2/determinism_hashes.json` matches.

...
def main(argv: list[str] | None = None) -> None:
    """CLI entry point for ``python -m flare``."""
    parser = argparse.ArgumentParser(
        prog="flare",
        description="FLARE — UAV navigation benchmark",
    )
    sub = parser.add_subparsers(dest="command")

    # --- run subcommand (RU-5) ---
    run_parser = sub.add_parser("run", help="Run benchmark episodes")
    run_parser.add_argument("--scenarios", type=str, default="osm_penteli_pharma_delivery_medium")

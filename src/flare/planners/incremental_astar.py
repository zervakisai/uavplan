"""Incremental A* planner (PL-5).

Renamed from D*Lite — uses A* internally with incremental obstacle-change
detection. Moderate risk coefficient γ=2.0.

Supports incremental replanning when obstacles change along the current path.
"""
...
from flare.blocking import SMOKE_BLOCKING_THRESHOLD, compute_blocking_mask
from flare.planners.astar import AStarPlanner
from flare.planners.base import PlannerBase, PlanResult

# Risk coefficient: moderate (incremental updates, balanced approach)
_RISK_GAMMA = 2.0


class IncrementalAStarPlanner(PlannerBase):
    """Incremental A* planner.
    ...
    Moderate risk: cost = 1.0 + γ * risk (γ=2.0).
    """
    ...
    def plan(
        ...
        Applies risk coefficient γ=2.0 (moderate).
        """
        ...
        if self._cached_mask is not None:
            ...
            result = inner.search(start, goal, cost_map=weighted)
        else:
            result = self._inner.search(start, goal, cost_map=weighted)
...
DStarLitePlanner = IncrementalAStarPlanner

"""Limited Visibility — partial observability filter (LV-1).
...
LV-1: Visibility filter is planner-agnostic. Same (pos, state, radius) → same
observation. All planners receive identical visibility-filtered state (FC-2).
"""
...
class LimitedVisibility:
    ...
    def observe(
        ...
        """Return visibility-filtered dynamic state.

        Visible cells (within sensor_radius of agent) = true values.
        Other cells = stale (last-known) values from memory.
        Deterministic: same (pos, state) → same observation (LV-1).
        """
        vis_mask = self._circle_mask(agent_xy)
        filtered: dict[str, Any] = {}

        # Keys that are spatial arrays requiring visibility filtering
        _SPATIAL_KEYS = (

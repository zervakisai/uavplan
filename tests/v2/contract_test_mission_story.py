...
Architecture notes
------------------
- Package: src/uavbench2/
- UrbanEnvV2 at src/uavbench2/envs/urban.py
...
- TerminationReason enum at src/uavbench2/envs/base.py
- service_time_s: consecutive STAY steps at POI to complete a task

Design intent
-------------
Tests are written spec-first (TDD).  They FAIL today because src/uavbench2
...
# ---------------------------------------------------------------------------
# Optional import: skip gracefully when uavbench2 is not yet installed.
...
uavbench2 = pytest.importorskip(
    "uavbench2",
    reason="uavbench2 is not installed — scaffold not yet implemented",
)
...
try:
    ...
    pytest.skip(f"uavbench2.envs.urban not available: {exc}", allow_module_level=True)

try:
    from uavbench2.envs.base import TerminationReason
except ImportError as exc:  # pragma: no cover
    pytest.skip(f"uavbench2.envs.base not available: {exc}", allow_module_level=True)

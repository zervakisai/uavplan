...
CC-2: Difficulty thresholds met (Medium>=50%, Hard>=15%)
...
class TestCC1_FeasibilityPreCheck:
    ...
    def test_returns_feasibility_result(self):
        ...
        config = load_scenario("gov_fire_delivery_easy")
        result = feasibility_pre_check(config, seed=42, horizon=20)
        assert isinstance(result, FeasibilityResult)
        assert result.seed == 42

    def test_easy_scenario_is_feasible(self):
        """Easy (no dynamics) should be feasible at step 0."""
        config = load_scenario("gov_fire_delivery_easy")
        result = feasibility_pre_check(config, seed=42, horizon=10)
        assert result.feasible, (
            "Easy scenario with no dynamics should be feasible"
        )
    ...
    def test_feasible_has_no_infeasible_step(self):
        ...
        config = load_scenario("gov_flood_rescue_easy")
        result = feasibility_pre_check(config, seed=42, horizon=10)
        if result.feasible:
            assert result.first_infeasible_step is None
    ...
    def test_infeasible_has_step_number(self):
        ...
        # Try multiple seeds — hard scenario may become infeasible
...
class TestCC2_DifficultyThresholds:
    ...
    def test_returns_calibration_result(self):
        ...
        result = calibrate_difficulty(
            "gov_fire_delivery_easy", n_seeds=3, horizon=10,
        )
        assert isinstance(result, CalibrationResult)
        assert result.n_seeds == 3
        assert 0.0 <= result.feasibility_rate <= 1.0

    def test_easy_passes_threshold(self):
        """Easy scenario should pass its calibration threshold."""
        result = calibrate_difficulty(
            "gov_fire_delivery_easy", n_seeds=5, horizon=10,
        )
        assert result.passes_threshold, (
            f"Easy scenario should pass threshold "
    ...
    def test_per_seed_results_populated(self):
        ...
        result = calibrate_difficulty(
            "gov_flood_rescue_easy", n_seeds=3, horizon=10,
        )
...
class TestCC4_InfeasibleFlagging:
    ...
    def test_infeasible_result_has_false_feasible(self):
        ...
        # Use a hard scenario with short horizon — more likely to hit infeasibility
        config = load_scenario("gov_fire_delivery_hard")

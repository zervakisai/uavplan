...
# ═════════════════════════════════════════════════════════════════════════════
# OperationalRenderer planned_path Tests (V&V Fix Issue 5)
# ═════════════════════════════════════════════════════════════════════════════

class TestOperationalRendererPlannedPath:
    """Verify that OperationalRenderer.render_frame accepts planned_path."""

    @pytest.fixture
    def op_renderer(self):
        import matplotlib
        matplotlib.use("Agg")
    ...
    def test_planned_path_parameter_accepted(self, op_renderer):
        """render_frame must accept planned_path without raising."""
        planned = [(5, 5), (10, 10), (15, 15), (20, 20), (25, 25)]
        frame = op_renderer.render_frame(
            drone_pos=(5, 5),
            step=3,
            trajectory=[(2, 2), (3, 3), (5, 5)],
            planned_path=planned,
        )
        assert frame.ndim == 3
        assert frame.shape[2] == 3
    ...
    def test_planned_path_none_does_not_crash(self, op_renderer):
        """render_frame must work when planned_path=None."""
        frame = op_renderer.render_frame(
            drone_pos=(10, 10),
            step=5,
            planned_path=None,
    ...
    def test_planned_path_single_point_skipped(self, op_renderer):
        """render_frame with a 1-element planned_path must not crash."""
        frame = op_renderer.render_frame(
            drone_pos=(10, 10),
            step=5,
            planned_path=[(10, 10)],
    ...
    def test_planned_path_empty_skipped(self, op_renderer):
        """render_frame with an empty planned_path must not crash."""
        frame = op_renderer.render_frame(
            drone_pos=(10, 10),
            step=5,
            planned_path=[],
    ...
    def test_plan_stale_when_no_planned_path(self, op_renderer):
        """Render sanity check: if plan_len > 1 the frame is valid;
        when plan_stale=True and planned_path is empty, HUD shows STALE
        (rendered without crashing) and returns a valid frame."""
        # Case 1: plan_len > 1, not stale — planned_path drawn
        frame_ok = op_renderer.render_frame(
            drone_pos=(5, 5),
            step=10,
            planned_path=[(6, 6), (7, 7), (8, 8)],
            plan_len=3,
            plan_stale=False,
            plan_reason="cadence",
        )
        assert frame_ok.ndim == 3 and frame_ok.shape[2] == 3

        # Case 2: plan_stale=True, planned_path empty — HUD must show STALE, no crash
        frame_stale = op_renderer.render_frame(
            drone_pos=(5, 5),
            step=11,
            planned_path=[],
            plan_len=3,
            plan_stale=True,
            plan_reason="budget_exceeded",
        )
        assert frame_stale.ndim == 3 and frame_stale.shape[2] == 3
